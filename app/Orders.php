...
class Orders extends Model
{
    protected $fillable = ['status', 'user_id', 'address_id', 'date', 'total_price', 'discount', 'shipping_rule',
        'shipping_rate', 'payment_method', 'shipping_role_id', 'note', 'payment_order_id','device_os','discount_code'];
    ...
    {
        $shipment = $this->hasOne(AramexShipment::class, 'order_id');
        return $shipment;
    }
...
//            ->select('order_items.id','order_id')
    ...
    {
        return $this->hasMany(Note::class, 'order_id')->select('note');
    }
}
