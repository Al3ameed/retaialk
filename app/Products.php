...
class Products extends Model
{
    ...
    protected $fillable = [
        ...
        'item_group', 'second_item_group', 'standard_rate', 'uom', 'weight', 'active',
        'stock_qty', 'brand_id', 'is_variant', 'parent_variant_id', 'has_variants',
        'user_deleted', 'is_deleted', 'has_attributes', 'is_food_extras',
        'food_taste_available', 'food_type', 'food_optional_note', 'cost', 'season_id',
        'slug_en', 'slug_ar', 'chart_image_filename'
    ];
    ...
    public function priceRuleRelation()
    {
        return $this->hasOne(PriceRules::class, 'product_id')
            ->where('valid_to', '>=', Carbon::now('Egypt'))
            ->where('valid_from', '<=', Carbon::now('Egypt'))
            ->with('itemPrice');
    }

    public function priceRuleRelationCMS()
    {
        return $this->hasOne(PriceRules::class, 'product_id')
            ->with('itemPrice');
    }
    ...
    public function chartImagePath($image)
    {
        return url('/public/imgs/products-chart-size/' . $image);
    }
    ...
    public function variantStockReport()
    {
        $main_warehouse = Warehouses::where('default_warehouse', 1)->first();
        return $this->belongsToMany(Warehouses::class, 'products_warehouses', 'product_id', 'warehouse_id')
            ->wherePivot('warehouse_id', $main_warehouse->id)
            ->select('projected_qty', 'name');
    }


    public function itemsInStock()
    {
        return $this->belongsTo(ItemWarehouse::class, 'id', 'product_id');
    }

    public function parent()
    {
        return $this->belongsTo(Products::class, 'parent_variant_id');
    }
    ...
    public function variantsProducts()
    {
        return $this->hasMany(Products::class, 'parent_variant_id')
            ...
            ->with(['variations', 'price', 'images', 'priceRuleRelation']);
    }
