...
class SalesController extends Controller
{
    ...
    public function ordersList()
    {
        ...
        $user_warehouseId = $user->warehouse_id;
        $districtIdArr = [];
        if ($user_warehouseId !== null) {
            $districts_id = Warehouses::where('id', $user_warehouseId)->pluck('district_id');
            $districtIdArr = json_decode($districts_id[0]);
        }
        ...
        $defaultTimeZone = new \DateTimeZone('Africa/Cairo');
        $gmtTimezone = new \DateTimeZone('GMT');

        foreach ($orders as $i => $order) {
            try {
                $orderDateTime = new \DateTime($order['date'], $gmtTimezone);
                $offset = $defaultTimeZone->getOffset($orderDateTime);
                $myInterval = \DateInterval::createFromDateString((string)$offset . 'seconds');
                $orderDateTime->add($myInterval);
                $date = $orderDateTime->format('Y-m-d H:i:s');
                $order['date'] = $date;

                if (count($districtIdArr) > 0) {
                    $orderAddressDistrictId = $order['address']->district_id;
                    if (!in_array($orderAddressDistrictId, $districtIdArr)) {
                        unset($orders[$i]);
                    }
                }
    ...
    public function details($id)
    {
        ...
        $sales_order = Orders::with('shipment')->findOrFail($id);
        ...
        $sales_order->status = optional($orderStatus)->status ?: $sales_order->status;
        ...
        $grand_total_amount = $sales_order->total_price;
        ...
        if($sales_order->discount_code != null || $sales_order->discount_code !=0){
          $promocode =   Promocode::where('code',$sales_order->discount_code)->first();
          if($promocode){
             if ($promocode->type != 'amount'){
                 $promocode_value = $promocode->reward.' '.'%';
             }else{
                 $promocode_value = $promocode->reward.' ';
             }
          }
        }
        $grand_total_amount += $sales_order->shipping_rate;
     ...
     //   $total_amount_after_discount = orderPromocodeDiscount($sales_order, $grand_total_amount, $shipping_rate);
        ...
        ($sales_order->shipping_rule == "Free Shipping")? $shipping = 0 : $shipping= $sales_order->shipping_rate;
        $total_amount_after_discount =  $sales_order->total_price + $shipping - $sales_order->discount;

        $defaultTimeZone = new \DateTimeZone('Africa/Cairo');
        $gmtTimezone = new \DateTimeZone('GMT');
        $orderDateTime = new \DateTime($sales_order['date'], $gmtTimezone);
        $offset = $defaultTimeZone->getOffset($orderDateTime);
        $myInterval = \DateInterval::createFromDateString((string)$offset . 'seconds');
        $orderDateTime->add($myInterval);
        $date = $orderDateTime->format('Y-m-d H:i:s');
        ...
        return view('admin.sales_orders.details', compact('total_money_of_returned',
            'parent_array', 'runsheets', 'districts', 'shipping_role', 'shipping_rate', 'sales_order',
            'items_list', 'grand_total_amount', 'item_details', 'user', 'address', 'total_amount_after_discount',
            'returned_parent_array', 'returned_items', 'orderHistoryArr', 'orderNotes','promocode_value'));
    ...
    public function getPromoCodeDetails(Request $request)
    {
        ...
        $request_headers[] = 'lang: ' . $lang;
        if(! $this->isMobile($request->header('User-Agent')) && Input::has('order') )
        {
           $order =  Orders::find(Input::get('order'));
           $user = User::where('id',$order->user_id)->first();
            $request_headers[] = 'token: ' . $user->token;
        }else{
            $request_headers[] = 'token: ' . $token;
        }
    ...
    public function edit($id)
    {
        ...
        if (isset($used_promo_code)) {
            ...
            if ($promocode) {
                ...
                } elseif ($promocode->type == 'persentage' || $promocode->type == 'percentage') {
        ...
        if (isset($shipping_role)) {
            return view('admin.sales_orders.edit_food', compact('discount_type',
                'admin_user','discount_amount', 'freeShipping', 'promo_code', 'selected_user_address_id', 'users', 'order', 'shipping_rate', 'grand_total_amount', 'total_amount_after_discount', 'time_sections', 'item_details', 'products', 'shippingRoles', 'shipping_role', 'parentProducts'));

        }
        return view('admin.sales_orders.edit_food', compact('discount_type', 'admin_user','discount_amount', 'freeShipping', 'promo_code', 'selected_user_address_id', 'users', 'order', 'shipping_rate', 'grand_total_amount', 'total_amount_after_discount', 'time_sections', 'item_details', 'products', 'shippingRoles', 'parentProducts'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($request, $id) {
            ...
            $order->shipping_rate = ShippingRule::find($request->shipping_role_id)->rate;
            ...
            $item_details = salesOrderDetails($orderItems, 0);
            $order->total_price = $item_details['grand_total_amount'];
            ...
            if ($request->has('order_total_price') && Auth::guard('admin_user')->user()->can('edit_order')) {
                $order->total_price = (int)$request->order_total_price;
            }

            if ($request->has('shipping_rate') && Auth::guard('admin_user')->user()->can('edit_order')) {
                $order->shipping_rate = (int)$request->shipping_rate;
            }
            ...
            $promocode_details = orderPromocodeDiscount($order, $order->total_price, $order->shipping_rate);
            $order->discount = $promocode_details['discount_amount'];
            $order->discount_code = $promocode_details['promocode'];
            $order->save();
        ...
        foreach ($orderids as $orderId) {
            ...
            if ($checkOrderExistance) {
                ...
                try {
                    ...
                    array_push($error , [
                        'message' => $ex,
                        'order_id'=> $checkOrderExistance->id
                    ]);
                }
            }
        }
        if (count($error) > 0) {
            Log::info('Failed to send updated order status  mail', $error);
            return redirect()->back()->withErrors('Updated status successfully but failed to send mail');
    ...
    public function changeOrderStatus(Request $request)
    {
        ...
        if ($checkOrderExistance) {
            ...
            try {
                ...
                return redirect()->back()->withErrors('Updated order status successfully but failed to send mail');
            }
        }
