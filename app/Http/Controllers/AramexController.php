...
{
    const SHIPPMENT_COMPANY = 'aramex';
    ...
    public function createShipment($id)
    {
        try {
                ...
                'Shipments' => array(
                    'Shipment' => array(
                        ...
                        'Consignee' => array(
                            ...
                            'AccountNumber' => '',
                            'PartyAddress' => array(
                                'Line1' => $user_address->title ? $user_address->title: 'Main',
            ...
            try {
                ...
                $fileName = substr($url, strrpos($auth_call->Shipments->ProcessedShipment->ShipmentLabel->LabelURL, '/') + 1);
                $path = public_path('aramexShipmentFiles');
                if (!file_exists($path)) {
                    mkdir($path, 0777, true);
                }
                $file = $path . '/'.$fileName;
                file_put_contents($file, $pdfContent);
                $savedURL = url('/public/aramexShipmentFiles/'. $fileName);
                ...
                $aramex_shipment->label_url = $savedURL;
                $aramex_shipment->response = json_encode($auth_call);
                $aramex_shipment->shipment_company = self::SHIPPMENT_COMPANY;
                $aramex_shipment->trackingNumber = '0';
