...
use Log;
use Mail;
use Modules\CmsMobile\Http\Controllers\CmsMobileController;
use Request;
use Response;
            ...
            if (empty($user)) {
                if ($lang == 'en') {
                    return Response::json([
                        'Status' => 'Error',
                        'message' => 'Unauthorized to get cart, Please login.'
                    ], 401);
                } else {
                    return Response::json([
                        'Status' => 'Error',
                        'message' => 'يجب عليك تسجيل الدخول اولاًً'
                    ], 401);
                }
            } else {
                $user_id = $user->id;
...
//                    dd(2, $checkMailExistance);
                ...
                $validator = Validator::make($request->all(), [
                    'phone' => 'required|min:7|max:18',
                ]);

                if ($validator->fails()) {
                    if ($lang == 'en') return Response::json([
                        'Status' => 'Error',
                        'message' => 'Please Enter Valid Phone Number'
                    ], 400);
                    return Response::json([
                        'Status' => 'Error',
                        'message' => 'من فضلك ادخل رقم تليفون صحيح'
                    ], 400);
                }
...
//                    $product->price = $item['price'] == null ? itemSellingPrice($product->id) : $item['price'];
                ...
                foreach ($newitemsarray as $newitem) {
                    ...
                    $finalitemsarray1['rate'] = $this->calculatePriceInCartItemProduct($newitem);
                    ...
                    $subTotal = $subTotal + ($newitem->qty * $this->calculatePriceInCartItemProduct($newitem));
                }
                ...
                if($payment_method=='Cash' || $payment_method=='Cash on delivery' || $payment_method='Credit Card'){
                    $data['lang'] = ($lang =='en')?'e':'a';
                    $data['user_mobile'] = $address->address_phone;
                    if($address->address_phone != null ){
                        (new CmsMobileController())->index($data, $order->id);
                    }
                }
                if ($weOrderId !== null) {
                ...
                if (isset($data['name']) && !$creditCard || ($creditCard && $creditCard !== 1)) {
                    try {
                        Mail::send(
                            ['html' => 'mail.mailTemplate'],
                            ['body' => $data, 'items' => $order_items],
            ...
            if ($promo_code) {
                $discount = $promo_code->type == 'amount'
                    ? $promo_code->reward
                    : (($promo_code->reward * $subTotal) / 100);
                $data['discount'] = $discount;
                $order->update(['discount' => $discount,'discount_code'=> $request->input('promocode')]);
            }


            if ($promo_code) {
                $promocode = Promocode::where('code', $request->input('promocode'))->first();
                $usedPromocode = UsedPromocode::create([
            ...
            if ($type == 'food') {
                ...
                if (!$product || !$item['id']) {
                    ...
                    if (array_key_exists('item_price_id', $item) && isset($item['item_price_id']) && !empty($item['item_price_id'])) {
                        ...
                        if (
                            $priceRule->valid_from > date('Y-m-d H:i:s')
                        ...
                        if (
                            $priceRule->min_qty > $item['qty']
                            || $priceRule->max_qty < $item['qty']
                        ) {
                            if ($lang == 'en') {
                                return Response::json([
                                    'Status' => 'Error',
                                    'message' => 'The quantity of the product is out of min and max for the offer'
                                ], 400);
                            } else {
    ...
    public function calculatePrice($cart)
    {
        ...
        $totalPrice = 0;
        foreach ($items as $item) {
            $product = Products::findOrFail($item->item_id);
            $price = getDiscountProduct($product->priceRuleRelation, $product->standard_rate);
            if ($price != null) {
                $totalPrice += $price['discounted_rate'] * $item->qty;
            } else {
                $totalPrice += $product->standard_rate * $item->qty;
            }
        }
        return $totalPrice;
    }

    public function calculatePriceInCartItemProduct($item)
    {
        ...
        $price = getDiscountProduct($product->priceRuleRelation, $product->standard_rate);
        if ($price != null) {
            $itemPrice += $price['discounted_rate'];
        } else {
            $itemPrice += $product->standard_rate;
        }
        return $itemPrice;
    }
    ...
    public function cartCalculations()
    {
        ...
        $validation = $this->cartCalculationsValidation();
        ...
        $total_price = $this->calculatePrice($cart);
        $discount = 0;
        $shipping_rule = calculateShippingRate($district, $cartItems);

        if (!is_array($shipping_rule)) {
            return $shipping_rule;
        }
        $shippingrate = $shipping_rule['rate'];
        $deliveryRole = $shipping_rule['shipping_rule'];

        if (request()->promocode) {
            $returnedValue = $this->checkoutPromocode(request()->promocode, $user->id, $total_price);
            if (!is_bool($returnedValue)) {
                return $returnedValue;
            }
            ...
            $discount = $promo_code->type == 'amount'
                ? $promo_code->reward
                : ($promo_code->reward * $total_price / 100);

            $shippingrate = $promo_code && $promo_code->freeShipping ? 0 : $shippingrate;
            $deliveryRole = $promo_code && $promo_code->freeShipping ? 'Free Shipping' : $deliveryRole;
        }

        $total_discount = $total_price - $discount;
        $vat = $total_discount < 0 ? $shippingrate * 0.14 : ($total_discount + $shippingrate) * 0.14;
        return response()->json([
            'total_price' => $total_price,
            'discount' => $discount > $total_price ? $total_price : $discount,
            'shipping_rate' => $shippingrate,
            'shipping_role' => $deliveryRole,
            // 'vat' => $vat,
            'final_total_price' => $total_discount < 0 ? $shippingrate : $total_discount + $shippingrate
        ], 200);
    }

    public function cartCalculationsValidation()
    {
        $token = getTokenFromReq(request());
        $lang = getLang();
        if (strpos($lang, 'ar') !== false) {
        ...
        // if (empty($address)) {
        //     if ($lang == 'en') {
        //         return response()->json(['Status' => 'Error', 'message' => 'No such address'], 401);
        //     } else {
        //         return response()->json(['Status' => 'Error', 'message' => "خطأ في العنوان"], 401);
        //     }
        // }
        ...
        // if (empty($district)) {
        //     if ($lang == 'en') {
        //         return response()->json(['Status' => 'Error', 'message' => 'No such district'], 401);
        //     } else {
        //         return response()->json(['Status' => 'Error', 'message' => "خطأ في العنوان"], 401);
        //     }
        // }
        ...
        if (empty($cart)) {
            if ($lang == 'en') {
                return response()->json(['Status' => 'Error', 'message' => 'No cart'], 401);
            } else {
                return response()->json(['Status' => 'Error', 'message' => "السلة فارغة"], 401);
            }
        }
    ...
    public function validateCode($code, $userId)
    {
        $promocodeRow = Promocode::where('code', $code)->first();
        // return $promocodeRow;
        if ($promocodeRow) {
            $checkUserPromocode = DB::table('user_promocode')->where('promocode_id', $promocodeRow->id)->where('user_id', $userId)->first();
        }
        if (is_null($promocodeRow)) {
            return -1;
        } elseif ($promocodeRow->userscount == 0 || $promocodeRow->active == 0) {
            return 0;
        } elseif (!is_null($promocodeRow->sto)) {
            if ($promocodeRow->sto < date('Y-m-d H:i:s')) {
                return 0;
            } else {
                return 1;
            }
        } elseif (isset($checkUserPromocode)) {
            return 2;
        } else {
            return 1;
        }
