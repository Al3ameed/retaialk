...
class CategoryApiController extends utilitiesController
{
        ...
        if (request()->has('id')) {
            ...
            $products = Products::whereIn('id', $products_id)
                ...
                ->select(
                    ...
                    'season_id','chart_image_filename'
                )
                ->latest('updated_at')
            ...
            // 1.0 check if the slug is not send to api
            if (!$category_slug) {
                if (getLang() == 'en') {
                    return Response::json(['Status' => 'Error', 'message' => "Category id is required"], 400);
                }
            ...
            // get category and all sub categories by lang and slug name
            $lang = getLang();
            $parent_category = Categories::where('slug_' . $lang, $category_slug)->first();
            ...
            // collect all category id and sub_ids in on variable
            $categories_id = $parent_category->childCategories()->pluck('id')->toArray();
            ...
            $products = Products::whereIn('id', $products_id)
                ...
                ->latest('updated_at')
                ->with('variantsProducts', 'images');
            if (authUser()) {
                $products->with('favourite');
            }
    ...
    public function getCategoriesHaveSub(Request $request)
    {
        try {
            updateUserDeviceData();
            $categories = Categories::orderBy('sorting_no', 'ASC')
                ->where('has_sub',1)
                ->where('status', 1)->get();
            $Categories_array = $this->getArrayCategories($categories);
