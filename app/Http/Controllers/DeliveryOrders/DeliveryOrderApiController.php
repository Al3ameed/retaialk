        ...
        if ($token != null) {
            ...
            if (empty($user)) {
                ...
                foreach ($user_oreders as $order) {
                    ...
                    $array_order[$i]["date"] = date("Y-m-d H:i:s", strtotime($order->date." +2 hours")) ;
                    ...
                    foreach ($order_items as $product) {
                        ...
                        $product_order[$x] = array_merge((array) $product, (array) $productArray);
                        if ($productInstance->priceRuleRelation !== null) {
                            if ($productInstance->priceRuleRelation->discount_type == 'percentage') {
                                $discount_tag = $productInstance->priceRuleRelation->discount_rate . '%';
                                $discounted_rate =  $productInstance->standard_rate - ($productInstance->standard_rate * ($productInstance->priceRuleRelation->discount_rate / 100));
                                $product_order[$x]['total_price'] = $productInstance->priceRuleRelation !== null ? ($discounted_rate * $product->qty) : $product->rate * $product->qty;
                            } else {
                                $discount_tag = $productInstance->priceRuleRelation->discount_rate . 'EGP';
                                $discounted_rate =  $productInstance->standard_rate -  $productInstance->priceRuleRelation->discount_rate;
                                $product_order[$x]['total_price'] = $productInstance->priceRuleRelation !== null ? ($discounted_rate * $product->qty)  : $product->rate * $product->qty;
                            }
                        } else {
                            $product_order[$x]['total_price'] = $product->rate * $product->qty;
                        }
                        $product_order[$x]['discount'] = $productInstance->priceRuleRelation ? [
                            'item_price_id' => $productInstance->priceRuleRelation->itemPrice->id,
                            'discount_tag' => $discount_tag,
                            'discounted_rate' => $discounted_rate,
                            'discount_rule' => $productInstance->priceRuleRelation->price_rule_name
                        ] : null;
                        $product_order[$x]['single_price'] = $product->rate;
                        $x++;
                    }
                    $cartTotalPrice = 0;
                    foreach ($product_order as $product) {
                       $cartTotalPrice += $product['total_price'] ;
                    }
...
//                    $final_total_price = $cartTotalPrice + $shipping_rate;
                    $final_total_price = $order->total_price + $shipping_rate;
                    $final_total_price = $final_total_price - $discount;
//                    $array_order[$i]['total_price'] = $cartTotalPrice;
    ...
    public function getUserOrdersDetails(Request $request, $id)
    {
        ...
        foreach ($user_oreders as $order) {
            ...
            $notes  = (object)Note::where('order_id',$order->id)->where('note','!=',null)->pluck('note');
            $notes[] = $order->note;
            ...
            $array_order["date"] = $order->date;
            $array_order["notes"] = $notes;

            $user_data = DB::table('users')->where('id', $order->user_id)
                ->select('id', 'email', 'name', 'phone')->first();
            $addresse = DB::table('address')->where('id', $order->address_id)->first();
            ...
            foreach ($order_items as $product) {
                ...
                if ($lang == 'en') {
                    $productArray = DB::table('products')->where(
                        'products.id',
                        $product->item_id
                    )
                        ...
                        ->select(
                            ...
                            'products.description_en as description',
                            'products.item_group',
                            'products.id',
                            'products.standard_rate',
                            'products.created_at',
                            'products.updated_at',
                            'uoms.type as uom'
                        )->first();
            ...
            foreach ($product_order as $cartItem) {
...
//            $final_total_price = $cartTotalPrice+ $shipping_rate;
            $final_total_price = $order->total_price + $shipping_rate;
            $final_total_price = $final_total_price - $discount;
//            $array_order['total_price'] = $cartTotalPrice;
            $array_order['total_price'] = $order->total_price;
            $array_order['discount'] = $discount;
