...
class ProductsController extends Controller
{
    ...
    public function productsList(ProductFilters $filter)
    {
        ...
        $products = Products::NotVariant()->with(['image', 'categories', 'price'])
            ->filter($filter)
            ->select('products.*')->latest('updated_at');
    ...
    public function store(AddProductRequest $request)
    {
        ...
        return DB::transaction(function () use ($request) {
            ...
            if ($request->hasFile('chartImages')) {
                $chartImages = $request->chartImages;
                if(isset($chartImages) && is_array($chartImages) && count($chartImages) > 0)
                    $chartImages = $chartImages[0];
                if (checkProductConfig('multi_images')) {
                    $file_name = saveImage($chartImages, "products-chart-size");
                    $product->chart_image_filename  = $file_name;
                    $product->save();
                }
            }
    ...
    public function edit($id)
    {
        ...
        $chart_images = getChartImage($product->chart_image_filename);
        $seasons = Seasons::all();
        ...
        $product_variationsList = Products::where('is_variant', 1)->where('parent_variant_id', $product->id)->get();
        $product_variations = getProductWithVariations($product_variationsList, 'en');
    ...
    public function addVariationView()
    {
        ...
        if ($has_special_images == 1) {
            ...
            $data = view('admin.products.variation_without_images', compact('variations_values', 'price', 'cost'))->render();
        }
        return response()->json(['data' => $data]);
    }
            ...
            foreach ($keys as $key => $value_key) {
                ...
                foreach ($variations as $key => $value) {
                    ...
                    $variant_option = VariantOption::create(
                        [
                            'variant_data_id' => $variation_object->id,
                            'variant_meta_id' => $variant->id
                        ]
                    );
                ...
                if (!$product_variation_exist) {
                    ...
                    $variant_product = Products::create([
                        'name' => $name_ar, 'name_en' => $name_en,
                        'description_en' => $product->description_en, 'description' => $product->description,
                        'item_group' => $product->item_group, 'item_code' => $product_bar_code,
                        'brand_id' => $product->brand_id, 'second_item_group' => $product->second_item_group,
                        'is_variant' => 1,
                        'parent_variant_id' => $product->id,
                        'standard_rate' => $price,
                        'cost' => $cost
                    ]);
                    $i++;
                    ...
                    $costPrice = ItemPrice::where('price_list_id', 2)
                        ->where('product_id', $variant_product->id)->first();
                    if ($costPrice) {
                        $costPrice->rate = $cost;
                        $costPrice->save();
                    } else {
                    ...
                    $variant_product = Products::where('id', $product_variation_exist->products_id)
                        ->first();
                    if (isset($request['variation_cost' . $int])) {
                        $cost = (float) $request['variation_cost' . $int];
                    }
