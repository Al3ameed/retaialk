...
class ProductsApiController extends Controller
{
        ...
        try {
            if (isset($_GET['id'])) {
                ...
                if ($product) {
                    ...
                    $product->standard_rate = itemSellingPriceWithDiscount($product->id);

                    putInCache('Product_' . $id, $product);
                    //                    $product = getProductWithVariations($product, $lang, );
                    if (is_array($product)) {
                        $product = $product[0];
                    }
    ...
    public function searchProducts()
    {
        ...
        })->where('is_variant', 0)->latest('updated_at');
    ...
    if (trim($lang) == 'en') {
        ...
        ->with('variantsProducts.variations', 'favourite', 'images')
        ->orderBy('name_en', 'asc')->latest('updated_at');
        ...
        ->with('variantsProducts.variations', 'favourite', 'images')
        ->orderBy('name', 'asc')->latest('updated_at');
        $products = $products->get();
    }

        $product_transformer = new ProductsTransformer;
        // $products_array = count($products) > 0 ? $products : [];
        $products_array = count($products) > 0 ? $product_transformer->transformCollection($products->toArray()) : [];
    ...
    public function getProducts()
    {
        try {
            ...
            $products = Products::where('products.is_bundle', 0)
        ...
        try {
            ...
            if (isset($_GET['id'])) {
                ...
                if (!$product) {
                    ...
                    $relatedProducts = Products::where('active', 1)->where('item_group', $product->item_group)->isVariant()->where('id', '!=', $product->id)->select('id', 'image', 'name_en', 'name', 'description_en', 'description', 'item_group', 'item_code', 'weight', 'has_attributes', 'brand_id', 'has_variants')->orderBy('sorting_no', 'desc')
                        ->latest('updated_at')
                        ->get();
        ...
        if (!empty($request->all())) {
            ...
            if (count($filters) <= 0) {
                ...
                if ($request->category != 0 && $cats) {
                    ...
                    if ($lang == 'en') {
                        $products = Products::where('active', 1)->scopeIsVariant()->select('id', 'name_en as name', 'slug_en as slug', 'image', 'description_en as description', 'has_variants', 'item_group', 'item_code', 'second_item_group', 'standard_rate', 'uom', 'has_attributes', 'weight', 'brand_id', 'chart_image_filename'
                        )->orderBy('sorting_no', 'desc')->latest('updated_at')->get();
                    } else {
                        $products = Products::where('active', 1)->scopeIsVariant()->select('id', 'name', 'slug_ar as slug', 'image', 'second_item_group', 'description', 'has_variants', 'item_group', 'item_code', 'standard_rate', 'uom', 'has_attributes', 'weight', 'brand_id', 'chart_image_filename'
                        )->orderBy('sorting_no', 'desc')->latest('updated_at')->get();
                    }
                }
                ...
                if ($request->category != 0 && $cats) {
                    ...
                    $query = Products::where('active', 1)->whereNull('deleted_at')->where('is_variant', 0)->latest('updated_at');
                }
    ...
    public function getProductByKey($key, $value)
    {
        $product = Products::where($key, $value)
            ...
            ->has('variantsProducts')
            ->with('favourite' , 'variantsProducts', 'images')
            ->first();
        if (!$product) {
            return;
        } else {
