...
function getProductWithVariations($products, $lang, $onlyAvailable = null)
{
    ...
    foreach ($products as  $key=>$product) {
        if($key !== true) {
            ...
            if ($product && isset($product->has_variants) && $product->has_variants == 1) {
                $variation_option = [];
                if ($lang == "en") {
                ...
                if ($product->priceRuleRelation !== null) {
                    if ($product->priceRuleRelation->discount_type == 'percentage') {
                        $discount_tag = $product->priceRuleRelation->discount_rate . '%';
                        $discounted_rate = $product->standard_rate - ($product->standard_rate * ($product->priceRuleRelation->discount_rate / 100));
                    } else {
                        $discount_tag = $product->priceRuleRelation->discount_rate . 'EGP';
                        $discounted_rate = $product->standard_rate - $product->priceRuleRelation->discount_rate;
                    }
                }
                $product->discount = $product->priceRuleRelation ? [
                    'item_price_id' => $product->priceRuleRelation->itemPrice? $product->priceRuleRelation->itemPrice->id: -1,
                    'discount_tag' => $discount_tag,
                    'discounted_rate' => $discounted_rate,
                    'discount_rule' => $product->priceRuleRelation->price_rule_name
                ] : null;
            ...
            if (isset($product) && isset($product->has_attributes) && $product->has_attributes == 1) {
...
function itemSellingPriceWithDiscount($product_id)
{
    $product = Products::where('id', $product_id)->first();
    if (checkProductConfig('maintaining_stocks') == true) {
        $item_rate = 0;
        ...
        if ($product && $product->is_variant == 1 && $item_rate == 0) {
            $parent = Products::where('id', $product->parent_variant_id)->first();
            $item_rate = itemSellingPriceWithDiscount($parent->id);
        }
    } else {
        $item_rate = $product->standard_rate;
    }
    $price_rule_relation = $product->priceRuleRelation;
    if ($price_rule_relation !== null) {
        if ($price_rule_relation['discount_type'] == 'percentage' || $price_rule_relation['discount_type'] == 'persentage') {
            $item_rate = $item_rate - ($item_rate * ($price_rule_relation['discount_rate'] / 100));
        } else {
            $item_rate =  $item_rate -  $price_rule_relation['discount_rate'];
        }
    }
    return $item_rate;
}
...
//        'item_price_id' => $product->priceRuleRelation->itemPrice->id,
    ...
    if (checkProductConfig('variations')) {
        ...
        $productWithVariations = getProductWithVariations($products, $lang);
        if (count($productWithVariations) > 0) {
            $product = $productWithVariations[0];
        } else {
            $product = $product;
        }
    }
...
{
    $productQty = 0;
    if ($warehouse && isset($product) && isset($warehouse->id) && isset($product->id)) {
...
// get discount price
function getDiscountProduct($price_rule_relation , $standard_rate) {
    $discount = null;
    if ($price_rule_relation !== null) {
...
function getCartItems($token, $items, $lang)
{
    ...
    foreach ($items as $item) {
        ...
        $dbProduct = \App\Products::where('id', '=', $item['item_id'])->select(
            'id',
            'name',
            'name_en',
            'description',
        ...
        if ($dbProduct == null) {
                DB::table('cart_items')->where('cart_id', $item['cart_id'])->where('item_id', $item['item_id'])->delete();
            continue;
        }
        ...
        if ($dbProduct) {
            ...
            $result = getDiscountProduct($dbProduct->priceRuleRelation , $dbProduct->standard_rate);
            $dbProduct->after_discount = $dbProduct->priceRuleRelation ? $item->price : null;
            $dbProduct->discount = $result;
            $dbProduct->stock_qty = $dbProduct->itemsInStock->projected_qty;
            if($dbProduct->stock_qty >0){
...
function getImagesVarientProduct($product_id)
{
    $product = Products::where('id', $product_id)->first();
    if ($product) {
        $url = array();
...
function savePdf($file, $type) {
    ...
    $pdf = \Barryvdh\DomPDF\PDF::make($file->getRealPath());
 ...
 function getChartImage($chartImage)
{
    $urls = [];
    $image_url = url('/public/imgs/products-chart-size/' . $chartImage);
    $urls[] = $image_url;
    return $urls;
}
...
function calculateShippingRate($district = null, $items)
{
    $weight = 0;
    foreach ($items as $item) {
