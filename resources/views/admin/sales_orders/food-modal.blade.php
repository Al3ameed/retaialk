        ...
        <div class="modal-body">
            <div class="form-group">
                <div class="row">
                    <div class="col-md-8" style="margin-left: 20px;">
                        <div class="row">
                            <label for=""><h3>Product Name: <span
                                            style="color: red">#</span><b>{{$product->name_en}}</b><label
                                            style="margin-left: 10px;"> (<span
                                                style="font-size: 20px;">{{$product->description_en}}</span>) </label>
                                </h3></label>
                        </div>
                    ...
                    <span class="input-group-btn">
                        <button type="button" onchange="grandTotal();" class="btn btn-default btn-number "
                                data-type="minus" data-field="quant[2]">
                          <span class="glyphicon glyphicon-minus"></span>
                        </button>
                    </span>
                            <input type="text" id="count" onchange="grandTotal();" readonly="readonly" name="quant[2]"
                                   class="form-control input-number" value="1" min="1" max="150">
                            <span class="input-group-btn">
                        <button type="button" class="btn btn-default btn-number " onchange="grandTotal();"
            ...
            @if($type != 'one_size')
                <div class="form-group">
                    <div class="row" id="choose"
                         style="margin:0px;background-color: #ccceb0;padding: 0px 10px;min-height: 50px;">
                    ...
                    @foreach($product_variations as $variant)
                        <div class="card card-block">
                            <div class="row" style="margin-bottom: 20px;">
                                <div class="col-lg-1">
                                    <input type="radio" value="{{$variant->item_id}}" class="option_type"
                                           id="option{{$variant->item_id}}" name="option">
                                    <input type="text" hidden="" disabled value="{{$variant->rate}}"
                                           id="item-price{{$variant->item_id}}">
                                    <input type="text" hidden="" name="food_type" value="{{$type}}">
                                    <input type="text" id="option_name{{$variant->item_id}}"
                                           value="{{$variant->item_name}}" hidden="">
                                </div>
                                <div class="col-lg-2" style="">
                                    <label style="margin-bottom: 0;" class="form-group"
                                           for="from">{{$variant->item_name}}
                                    </label>
                                </div>
                                <div class="col-lg-6" style="float: right;left: 50px;">
                                    <div class="col-lg-3 div_prices" style="width: 150px;display: none;"
                                         id="div_price{{$variant->item_id}}">
                                        <label for="">Price</label>
                                        <input type="" readonly="" class="form-control" value="{{$variant->rate}} LE">
                                    </div>
                                    <div class="col-lg-3 prices" style="width: 150px;display: none;"
                                         id="price-{{$variant->item_id}}">
                                        <label for="">Total</label>
                                        <input type="text" class="form-control" id="total_price-{{$variant->item_id}}"
                                               readonly="" value="0">
                                    </div>
                                </div>
                            ...
                            @if(count($variant->extras) >0)
                                <div class="row extras" id="extra-{{$variant->item_id}}"
                                     style="display:none;margin-left: 50px;margin-bottom: 20px;">
                                        ...
                                        <table style="">
                                            ...
                                            @foreach($variant->extras as $extra)
                                                <tr class="tr-{{$variant->item_id}}" id="extra_field{{$i}}">
                                                    <td class="col-lg-3">
                                                        <input type="text" disabled="" value="{{$extra->name}}"
                                                               id="extra_name{{$i}}" class="form-control">

                                                        <input type="text" class="extras-{{$variant->item_id}}"
                                                               hidden="" id="extra_id{{$i}}"
                                                               name="extra_id-{{$variant->item_id}}[]"
                                                               value="{{$extra->extra_id}}">
                                                    </td>
                                                    <td class="col-lg-2">
                                                        <input type="number" value="1" id="extra_qty{{$i}}" min="1"
                                                               onchange="grandTotal();"
                                                               name="qty-{{$variant->item_id}}[]"
                                                               class="form-control extra_qty">
                                                    </td>

                                                    <td class="col-lg-2">
                                                        <input type="text" disabled=""
                                                               value="{{$extra->food_extra_price}}"
                                                               id="extra_rate{{$i}}" class="form-control">
                                                    </td>
            ...
            @else
                ...
                <div class="card card-block">
                    <div class="row" style="margin-bottom: 20px;">
                        <div class="col-lg-1">
                            <input type="radio" checked="" readonly="" value="{{$product->id}}" class="option_type"
                                   id="option{{$product->id}}" name="option">
                            <input type="text" hidden="" disabled value="{{$item_rate}}"
                        ...
                        <div class="col-lg-6" style="float: right;left: 50px;">
                            ...
                            <div class="col-lg-3 prices" style="width: 150px;" id="price-{{$product->id}}">
                                <label for="">Total</label>
                                <input type="text" class="form-control" id="total_price-{{$product->id}}" readonly=""
                                       value="0 LE">
                            </div>
                        </div>
                    </div>
                                ...
                                @foreach($extras_array as $extra)
                                    <tr class="tr-{{$product->id}}" id="extra_field{{$i}}">
                                        <td class="col-lg-3">
                                            ...
                                            <input type="text" class="extras-{{$product->id}}" hidden=""
                                                   id="extra_id{{$i}}" name="extra_id-{{$product->id}}[]"
                                                   value="{{$extra->extra_id}}">
                                        </td>
                                        <td class="col-lg-2">
                                            <input type="number" value="1" id="extra_qty{{$i}}" min="1"
                                                   onchange="grandTotal();" name="qty-{{$product->id}}[]"
                                                   class="form-control extra_qty">
                                        </td>

                                        <td class="col-lg-2">
                                            <input type="text" disabled="" value="{{$extra->food_extra_price}}"
                                                   id="extra_rate{{$i}}" class="form-control">
                                        </td>
            ...
            <div class="row">
                <div class="col-sm-32" id="saved">
                    <button type="button" id="cancel" style="margin-left: 12px" class="btn btn-warning cancel">Cancel
                    </button>
