...
<body class="fixed-left">
        ...
        <div class="content-page">
            ...
            <div class="content">
                <div class="container">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <a class="btn btn-sm btn-danger" href="javascript:void(0)" id="delItem"
                                        title="Hapus"><i class="glyphicon glyphicon-trash"></i> Delete Item</a>
                    ...
                    <div class="modal fade bs-example-modal-sm" id="shippingModal" tabindex="-1" role="dialog"
                        aria-labelledby="mySmallModalLabel">
                        <div class="modal-dialog modal-sm" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <div class="modal-body">
                                    ...
                                    <select class="form-control" name="shipping-select" id="shippingSelect">
                                        <option value="-1" selected disabled>Choose Shipping Company</option>
                                        <option value="1">Aramex</option>
                                        <option value="2">Bosta</option>
                                        <option value="3">Mylerz</option>
                                    </select>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" id="btnSubmitShipment" class="btn btn-primary"
                    ...
                    <div class="row">
                        ...
                        <div class="card card-block">
                            <div class="card-title">
                                <div class="row">
                                    <div class="col-sm-2">
                                        <a href="{{url('/admin/sales-orders/create')}}"
                                            class="btn btn-rounded btn-primary"><i class="zmdi zmdi-plus-circle-o"></i>
                                            Add New Order</a>
                                    </div>
                                    ...
                                    <div class="col-sm-3">
                                        ...
                                        <div class="col-sm-8">
                                            <select id="product_id" class="form-control select2">
                                                ...
                                                <option value="{{$product->id}}" @if($product_id==$product->id)selected
                                    ...
                                    <div class="col-sm-3">
                                        <div class="col-sm-4" style="margin-top: 7px;">
                                            <label for="crewdateofjoining">Status: <span
                                                    style="color:red;"></span></label>
                                        </div>
                                        <div class="col-sm-8">
                            ...
                            <form action="{{url('admin/updateOrderStatus')}}" method="POST">
                                ...
                                <div class="card-text">
                                    ...
                                    <table id="items_datatable" class="table table-striped table-bordered"
                                    ...
                                    <div class="row">
                                        <div class="col-sm-4">Update Orders Status
                            ...
                            <button class="btn btn-sm btn-primary" id="btnDownloadPdf">Download Shipment
                                Receipt</button>

                            <div class="row">
                                <form action="{{route('export.csv.sales.filter')}}" method="POST">
                                    {!! csrf_field() !!}
                                    <div class="card-text">
                                        <div class="row">
                                            <div class="col-sm-4">
                                            ...
                                            <div class="col-sm-4">
                                                <input type="submit" class="btn btn-success" value="Export orders"></div>
                                            </div>
                                        </div>
                                    </div>
                                </form>
    ...
    <script>
        ...
        <?php $aramexShipmentUrl = url('/admin/sales-orders/');?>
        <?php $aramexTrackUrl = url('/admin/sales-orders/');?>

        <?php $bostaShipmentUrl = url('/admin/bosta/shipment/create');?>
        <?php $bostaTrackUrl = url('/admin/bosta/shipment/track');?>
        
        <?php $mylerzShipmentUrl = url('/admin/mylerz/shipment/create');?>
        <?php $mylerzTrackUrl = url('/admin/mylerz/shipment/track');?>

        <?php $shipmentReceiptsUrl = url('/api/shipment/receipt/download');?>
    ...
    var table = $('#items_datatable').DataTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    console.log("TCL: data", data, data2)
                        ...
                        {{--}--}}
                        if(!data) {
                            return "<a data-toggle='modal' data-target='#shippingModal' class='btn btn-secondary openShipment' data-id=" + type.id +"> " +
                            "<i class='fas fa-truck' id='shipment' ></i> </a> &nbsp;";
                        } else {
                            if (data.shipment_company == 'aramex') {
                                return "<a class='btn btn-dark' href='{{$aramexTrackUrl}}/" + data.shipment_id + "/shipment/track'> " +
                                    "<i class='fas fa-road' data-toggle='tooltip' data-placement='top' title='' id='track' ></i> </a> &nbsp;"
                            } else if (data.shipment_company == 'bosta') {
                                return "<a class='btn btn-dark' href='{{$bostaTrackUrl}}?id=" + data.shipment_id + "'> " +
                                    "<i class='fas fa-road' data-toggle='tooltip' data-placement='top' title='' id='track' ></i> </a> &nbsp;"
                            } else {
                                return "<a class='btn btn-dark' href='{{$mylerzTrackUrl}}?id=" + data.shipment_id + "'> " +
                                    "<i class='fas fa-road' data-toggle='tooltip' data-placement='top' title='' id='track' ></i> </a> &nbsp;"
                            }
    ...
    $('#btnSubmitShipment').on('click', function () {
        const orderId = $('#orderId').val();
        const shippingOption = $('#shippingSelect').val();

        if (shippingOption == 1) {
            window.location.href = '{{$aramexShipmentUrl}}/'+ orderId +'/shipment/create';
        } else if (shippingOption == 2) {
            window.location.href = '{{$bostaShipmentUrl}}?id='+ orderId;
        } else if (shippingOption == 3) {
            window.location.href = '{{$mylerzShipmentUrl}}?id='+ orderId;
        }
     });

        $(document).on("click", ".openShipment", function () {
...
// TODO : GET checkbox ids
    $(document).on('click', '#btnDownloadPdf', function () {
     var value =  [];

     $.each($("input[name='orderids[]']:checked"), function(){
        value.push($(this).val());
            });

            $.ajax({
            url: '{{$shipmentReceiptsUrl}}',
            type: "POST",
            dataType:"JSON",
            data: {'order_ids': value},
            success: function (response) {
            console.log("TCL: response", response);
                //if success reload ajax table

                var pdfs = response;
                console.log('pds: ', pdfs);
                pdfs.forEach(pdfURL => {
                    console.log('OI: ', pdfURL);
